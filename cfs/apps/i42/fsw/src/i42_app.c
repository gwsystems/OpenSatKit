...
//   CMDMGR_RegisterFunc(CMDMGR_OBJ, I42_NETIF_DISCONNECT_42_CMD_FC, NETIF_OBJ, NETIF42_Disconnect42Cmd, NETIF_DISCONNECT_42_CMD_DATA_LEN);
   ...
   /*
    * Sensor dump queue could be created by the handler thread if it runs first.
    * Here check if it exists, if not, create it.
   ...
   if (Status != OS_SUCCESS) {
         OS_printf("Failed to get sensor dump queue[%d], creating it.\n", Status);
   }
...
//   if (I42App.NetIf.Connected == TRUE) {
   if (sensordump_qid >= 0) {
      I42App.ConnectCycleCnt++;
          ...
          Status = OS_QueueGet(sensordump_qid, I42App.InBuf, I42_SENSOREMU_DATASZ, &copied, OS_PEND);
...
static boolean SendActuatorPkt(const char* Buf, uint16 Len) {
   ...
   Status = 1;
   /* NOTE: keep in mind that Composite printc may not be able to print beyond a certain length! */
   //OS_printf("Actuator dump: %d=%s\n", Len, Buf);
